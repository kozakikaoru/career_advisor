 ...
 *   ラッパを外し、内側コンテンツだけ返す。PlanContent 側で外側 1 枚の glow-card に内包される。
 * - bare=false(デフォルト)時は従来通り単独カードとして描画(後方互換)。
 */
export function CandidateHeader({
  ...
  if (bare) {
    // 統合カード内に置かれる用。外側ラッパなしで内部のみ返す。
    return <div>{inner}</div>;
  }

  return (
    <section className="mb-8">
      <div className="glow-card rounded-3xl p-6 sm:p-8">{inner}</div>
    </section>
  );
}
