...
export function buildPrompt(answers: AnswerMap): string {
  ...
  return [
    ...
    "- 医学部・看護学部・薬学部 → 医療・看護・介護を最優先",
    "- 情報工学科・情報系大学院 → IT・Web / ソフトウェア開発 / データ・AI を最優先",
    "- 法学部・法科大学院 → 法律・行政を最優先",
    "- 経営・経済学部 → 金融・会計 / 営業 / マーケティングを候補に",
    "- 文学部 → 教育 / メディア・出版 / 語学 を候補に",
    "- 教育学部 → 教育・保育を最優先",
    "- 工学部 → 製造・エンジニアリング / 研究・学術を最優先",
    "- 建築学科 → 建築・土木を最優先",
    "- 芸術系 → デザイン・クリエイティブ / 芸術・音楽 / メディアを候補に",
    "- 農学部 → 農林水産 / 研究・学術 を候補に",
    "## 分野クロス志向の見落とし禁止(複数候補の並列提示)",
    ...
    "",
    // ============================================================
    // MINDSET v2 解釈ガイド(短縮版)
    // specs/mindset-questions-v2.md §7-1 / §7-2 / §7-3 / §7-4
    // ============================================================
    "# 解釈の前提: MINDSET v2 — 性格傾向の進路提案への反映",
    "## ビッグファイブ的シグナルの扱い(§7-1)",
    "- 後段の性格傾向セクション(5 軸 low/mid/high)は、進路文脈の MINDSET 回答から AI が暗黙的に推定した参考シグナル(性格テストの結果ではない)。",
    "- AI 内部での 3 案組み立てに使う。結果画面の Plan 説明・ロードマップ文言には『進路文脈の言葉』で柔らかく織り込み、ビッグファイブの軸名(外向性 / 協調性 / 誠実性 / 神経症傾向 / 開放性)・MBTI 等の類型名・数値スコアそのものは出さないこと。",
    "- 性格を強く断定しない(『あなたは ○○ な人です』NG)。代わりに『○○志向が見えます』『○○ なタイプには ×× が向きやすい』のように柔らかく言及する。",
    "- `unknown_field_jump` / `competition_pref` / `failure_recovery` で「neither」=「明確な傾向なし・中庸」と解釈。「答えない」とは別物・意味のある中庸シグナル。",
    "- meaning_priority と value_priority の組合せで社会的意義への重みを判定する(value=meaning + meaning_priority=meaning → 強い意味志向、value≠meaning + meaning_priority=success → 強い成功志向)。",
    ...
    "- 各 mustLearn は title(40 字以内)+ description(120 字以内)。「なぜ学ぶか」を一言含む。",
    ...
    "- 基本は **8 段固定**: NOW / 3M / 6M / 1Y / 2Y / 3Y / 5Y / GOAL。",
    "- 短縮可: GOAL.goal_horizon=1y のような短期 → 3〜6 段に / 単純すぎる進路 → 3〜5 段に。下限 3、上限 8(schema 制約)。",
    ...
    "- 各 RoadmapNode.description には**具体的な数値・期限・手段**を含める(40 字以上 220 字以下)。最初のノード kind=\"start\"、最後 kind=\"goal\"、途中 kind=\"milestone\"。",
    "",
    ...
    "- feasibility != realistic のとき warning に 20〜160 字で「なぜ厳しいか」+「やるなら何が必要か / 代替案」を書く。",
    "- トーン規範:",
    ...
    "- 「努力次第で達成可能」のような楽観バイアスは避ける。難易度の高さを率直に伝えつつ「やるなら覚悟して」と背中を押すトーンが望ましい。extreme_effort は UI 上「超努力が必要」と表示される。突き放しではなく「やるなら覚悟がいる」前提で warning を書く。",
    "",
    ...
    "- 「努力次第で達成可能」「やればできる」のような楽観バイアスを排除する。難易度の高さを率直に伝えるのが配慮ある誠実さ。",
    "- 未確定情報を理由に逃げない(§8-5-2 (i-3) と同じ原則。「業界が未定なので分かりません」と返さず、進学先・知見・MINDSET から具体候補を推論する)。",
    "- ただし決めつけは避ける。「あなたは絶対 X すべき」ではなく「X という選択肢がある / Y の方が現実的かもしれない」と複数提示。",
    ...
    "- トーンは前向きで具体的。占い的な決めつけや、医療・法律・投資の助言と誤解される表現は避ける。個人特定情報は結果に含めない。",
  ].join("\n");
}
